...
const headersString = (value: string, example?: string) => `Prefer: code=${value} ${example ? `, example=${example}` : ''}`
...
export const KeyValueField = ({
  ...
  const renderExamplesBlock = (responses: TResponse[]) => {
    return <RadioWrapper>{responses.map((response) => {
      const selectOption = response.examples.map(item => <option value={item}>{item}</option>);
      const preferCode = response.code;
      return <RadioWrapper key={id}>
        <RadioGroup
          id='code'
        ...
        {response.examples.length > 0 && currCode === preferCode && <>
          <select id="example" onChange={(event) => {
            setValue(
              headersString(preferCode, event.target.value)
            )
          }}>
            <option selected disabled>Выберите ответ</option>
            {selectOption}
          </select>
        </>
        }
